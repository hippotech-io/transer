...
- **Cookieによって内容が変わるページのため、明示的なキャッシュ無効化ヘッダー
  （`Cache-Control: no-store`等）をサンプルコードに追加。**

### 移行時の注意（v0.3.1以前からのアップグレード）

既存の翻訳プロキシに、原文（日本語）ページへの言語ボックス挿入処理が
無い場合、上記サンプルの`else`節（`LANGBOX_SNIPPET`の挿入部分）を
...
<script src="https://api.transer.io/js/langbox.js?lang=ja" charset="utf-8"></script>
```
...
LANGBOX_SNIPPET = f'<script src="https://api.transer.io/js/langbox.js?lang=ja" charset="utf-8"></script>'
...
async def startup_selftest():
    ...
    try:
        ...
        print(f"[起動時セルフテスト] ✓ 成功: {result[:80]}")
    except Exception as e:
        print(f"[起動時セルフテスト] ✗ 失敗: {e}")
        print("  → api_key・hostname・「ドメイン管理」での契約言語登録を確認してください")
...
async def proxy(request: Request, path: str):
    ...
    if lang != "ja":
        ...
        if "</head>" in html:
            html = html.replace("</head>", LANGBOX_SNIPPET + "</head>", 1)

    # Cookie（transer_lang）によって内容が変わるページのため、ブラウザ・中間プロキシに
    # 古い言語のページがキャッシュされないよう、明示的にキャッシュを無効化する。
    return HTMLResponse(
        html,
        status_code=origin_resp.status_code,
        headers={"Cache-Control": "no-store, no-cache, must-revalidate", "Vary": "Cookie"},
    )
```
...
5. `curl -b "transer_lang=en" http://127.0.0.1:8090/` — 実際に翻訳された内容が返るか確認
...
`LANGBOX_SNIPPET`により、プロキシ自体が原文ページの`</head>`直前に
...
契約している翻訳先言語一覧は、`langbox.js`側がリクエスト元のドメインから
自動的に判定するため、指定する必要はありません。
...
| 言語ボックスが原文ページに出ない | サンプルコードの`LANGBOX_SNIPPET`挿入部分（`else`節）が反映されているか確認。ブラウザのNetworkタブで`langbox.js`が読み込まれているか（404になっていないか）確認 |
